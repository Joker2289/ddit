...
public class Ex5 {
	...
	public static void main(String[] args) {
		...
		b. int[] arr = {1,2,3,};
		c. int[] arr = new int[5];
		d. int[] arr = new int[5]{1,2,3,4,5};	- 크기 선언시 대괄호에 값을 주면안됨
		e. int arr[5];							- 배열 선언시 크기를 주면안됨
		f. int[] arr[] = new int[3][];*/
 		...
 		int money = 2680;
 			...
 			for(int i=0;i<coinUnit.length;i++){
 				System.out.println(coinUnit[i]+"원 = "+money/coinUnit[i]);
 			 	money = money % coinUnit[i];	// 1번 
 				
 				int tmp=0;
 				tmp=money/coinUnit[i];
 				money -= (tmp*coinUnit[i]);
 				System.out.println(coinUnit[i]+"원 = " + tmp);	//2번 
	...
	/*	
		...
		/*[연습문제]/ch5/Exercise5_8.java
				...
				public static void main(String[] args) {{*/
					...
					for(int i=0; i < answer.length;i++) {
						counter[answer[i]-1]++;	// answer에 들어있는값을 배열주소로 주고 								
					}							// 1씩증가시키면됨
						for(int i=0; i<counter.length;i++) {
							System.out.print(counter[i]);
							for(int j=0; j<counter[i]; j++){  // 인덱스에 들어있는값만큼 별을찎어줌
								System.out.print("*");
							}
							System.out.println();
						}*/
		...
		//에 알맞은 코드를 넣어서 완성하시오		
			...
			//[5-12] 예제5-23을 변경하여, 아래와 같은 결과가 나오도록 하시오.
				...
				System.out.printf("총 %s문제중 %d문제 맞추셨습니다", words.length, count);*/
					...
					public static void main(String args[]) {*/
						...
						char[] question = words[i].toCharArray(); // String을 char[]로 변환
							for(int j = 0; j<10; j++){
								...
								char word = question[tmp];
								question[tmp] = question[i];	
								question[i] = word;	
							}
