...
public class ExceptionHandling {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * 2.런타임 에러
		 * - 에러 : 프로그램 코드에 의해서 수습될 수 없는 심각한 오류	/ ex)메모리 부족
		 * - 예외 : 프로그램 코드에 의해서 수습될 수 있는 다소 미약한 오류
		 * - 런타임 에러가 발생 시 발생한 위치에서 프로그램이 비정상적으로 종료된다.
		 * 
		 * 3.예외 
		 * - 모든 예외는 Exception의 자식 클래스이다
		 * - RuntimeException 클래스와 그 자식 클래스 외의 모든 예외는 예외처리가 강제된다
		 * - 예외처리가 강제되는 예외는 'checked예외'
		 * 	 그렇지 않은 예외는 'unchecked 예외' 라고 한다.
		 * 
		 * 4.예외처리(try-catch)
		 * - 예외처리를 통행 프로그램이 비정상적으로 종료되는 것을 방지할 수 있다.
		...
		try {
			int result = 10 / 0; // 예외가 발생한 위치에서 프로그램이 비정상적을 종료된다
			System.out.println(result); // 예외 발생 시 실행되지 않는다
		} catch(ArithmeticException | IndexOutOfBoundsException e ){	//여러개의 예외를 같이 
			//예외 발생 시 발생한 예외와 일치하는 catch블럭이 수행되고				//처리하고 싶을때 ' | '
			//일치하는 catch가 없는 경우 예외는 처리되지 않는다
		} catch(NullPointerException e){
			//여러 종류의 예외를 처리하기 위해 catch를 하나 이상 사용할 수 있다.
		}
		// 변수 e에는 try에서 발생안 에러의 객체가 담겨 있다
		// e는 지역변수 이기 때문에 변수명은 중복이 가능하다
